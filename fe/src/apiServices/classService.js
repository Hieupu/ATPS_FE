...
const classService = {
  ...
  getAllTimeslots: async () => {
    try {
      ...
      // Đảm bảo mỗi timeslot có trường Day (backward compatibility)
      // Nếu backend chưa có Day, sẽ tự động thêm Day mặc định dựa trên logic
      return timeslots.map(ts => {
        if (!ts.Day && !ts.day) {
          // Nếu không có Day, có thể để null hoặc set mặc định
          // Frontend sẽ xử lý khi hiển thị
          return { ...ts, Day: null };
        }
        return ts;
      });
      ...
      // 404/403: Endpoint không tồn tại hoặc không có quyền
        ...
        return [
          {
            ...
            StartTime: "08:00:00",
            EndTime: "10:00:00",
            Day: "T2", // Thêm Day mặc định cho mock data
          },
          {
            TimeslotID: 2,
            StartTime: "14:00:00",
      ...
      // 500: Lỗi server - có thể do backend chưa hỗ trợ trường Day
      if (status === 500) {
        console.error("Get timeslots error (500): Backend server error. Possible causes:");
        ...
        // Trả về empty array để tránh crash, nhưng log lỗi rõ ràng
        // Frontend có thể hiển thị thông báo lỗi cho user
        return [];
      }
  ...
  getClassSessionsForFrontend: async (classId) => {
    try {
      const response = await apiClient.get(`/timeslots/class/${classId}/sessions`);
      return response.data?.data || response.data || [];
    } catch (error) {
  ...
  getSessionsByInstructorId: async (instructorId) => {
    try {
      const response = await apiClient.get(`/sessions/instructor/${instructorId}`);
      return response.data?.data || response.data || [];
    } catch (error) {
      console.error("Get sessions by instructor error:", error);
      throw error.response?.data || { message: "Failed to fetch instructor sessions" };
    }
  },
  ...
  bulkCreateSessions: async (sessionsData) => {
    try {
      console.log("Sending bulk create request with", sessionsData.length, "sessions");
      ...
      console.log("Bulk create API response (full):", JSON.stringify(response.data, null, 2));
      console.log("Bulk create API response keys:", Object.keys(response.data || {}));
      ...
      if (isConflictError) {
        ...
        while ((match = sessionPattern.exec(errorMessage)) !== null) {
          ...
          if (conflictText) {
            ...
            const timeMatch = conflictText.match(/(\d{2}:\d{2}:\d{2})\s*-\s*(\d{2}:\d{2}:\d{2})/);
            
            conflicts.push({
              sessionIndex: sessionIndex,
              conflictType: conflictText.includes("Instructor") ? "instructor" : "class",
              conflictInfo: {
        ...
        if (conflicts.length === 0 && errorMessage) {
          conflicts.push({
            sessionIndex: 1,
            conflictType: errorMessage.includes("Instructor") ? "instructor" : "class",
            conflictInfo: {
      ...
      throw error.response?.data || { message: errorMessage || "Failed to bulk create sessions" };
    }
  },
  ...
  getSessionsByDateRange: async (startDate, endDate) => {
    try {
      ...
      throw error.response?.data || { message: "Failed to fetch sessions by date range" };
    }
  },
  ...
  getSessionsByTimeslotId: async (timeslotId) => {
    try {
      ...
      throw error.response?.data || { message: "Failed to fetch sessions by timeslot" };
    }
  },

  // Gửi cho giảng viên chuẩn bị (Submit for approval)
    ...
    try {
      ...
      if (error.response?.status === 404) {
        ...
        try {
          ...
          console.error("Submit for approval error (updateClass):", updateError);
          throw (
            updateError.response?.data || { message: "Failed to submit for approval" }
          );
        }
      } else {
        console.error("Submit for approval error:", error);
        throw (
          error.response?.data || { message: error.response?.data?.message || "Failed to submit for approval" }
        );
      }
    }
  },
  ...
  reviewClass: async (classId, action, adminFeedback = "") => {
    try {
      ...
      if (error.response?.status === 404) {
        ...
        throw error.response?.data || { message: error.response?.data?.message || "Failed to review class" };
      }
    }
  },
  ...
  publishClass: async (classId) => {
    try {
      ...
      if (error.response?.status === 404) {
        ...
        throw error.response?.data || { message: error.response?.data?.message || "Failed to publish class" };
      }
    }
  },
  ...
  createClassWizard: async (wizardData) => {
    try {
      ...
      throw error.response?.data || { message: "Failed to create class with wizard" };
    }
  },
  ...
  findAvailableInstructorSlots: async (instructorId, timeslotId, day, numSuggestions = 5) => {
    try {
      const params = new URLSearchParams({
      ...
      const response = await apiClient.get(`/classes/instructor/available-slots?${params}`);
      return response.data?.data || response.data;
    } catch (error) {
      console.error("Find available instructor slots error:", error);
      throw error.response?.data || { message: "Failed to find available slots" };
    }
  },
  ...
  checkLearnerConflicts: async (classId, date, timeslotId) => {
    try {
      const response = await apiClient.post("/classes/check-learner-conflicts", {
        ClassID: classId,
        Date: date,
      ...
      throw error.response?.data || { message: "Failed to check learner conflicts" };
    }
  },
  ...
  addBulkInstructorLeave: async (leaveData) => {
    try {
      const response = await apiClient.post("/classes/instructor/leave/bulk", leaveData);
      return response.data?.data || response.data;
    } catch (error) {
      console.error("Add bulk instructor leave error:", error);
      throw error.response?.data || { message: "Failed to add instructor leave" };
    }
  },
  ...
  checkFutureConflicts: async (instructorId, date, timeslotId, status) => {
    try {
      const response = await apiClient.post("/classes/instructor/leave/check-conflicts", {
      ...
      throw error.response?.data || { message: "Failed to check future conflicts" };
    }
  },

  // Xử lý lớp bị ảnh hưởng
  handleAffectedClass: async (classId, action, sessionIds, newSchedule = null) => {
    try {
      const requestData = {
        ClassID: classId,
      ...
      const response = await apiClient.post(`/classes/${classId}/handle-affected`, requestData);
      return response.data?.data || response.data;
    } catch (error) {
      console.error("Handle affected class error:", error);
      throw error.response?.data || { message: "Failed to handle affected class" };
    }
  },
  ...
  countLearners: async (classId) => {
    try {
      const response = await apiClient.get(`/classes/${classId}/learners/count`);
      return response.data?.data || response.data;
    } catch (error) {
  ...
  addMakeupSession: async (classId, sessionData) => {
    try {
      const response = await apiClient.post(`/classes/${classId}/sessions/makeup`, sessionData);
      return response.data?.data || response.data;
    } catch (error) {
      console.error("Add makeup session error:", error);
      throw error.response?.data || { message: "Failed to add makeup session" };
