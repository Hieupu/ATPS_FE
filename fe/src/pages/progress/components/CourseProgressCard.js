...
const CourseProgressCard = ({ item, index }) => {
  ...
  const examProgress =
    item.stats.totalExams > 0
      ? (item.stats.completedExams / item.stats.totalExams) * 100
      : 0;

  // Kiểm tra xem khóa học đã hoàn thành chưa
  const isCourseCompleted = item.classesDetail?.some(
    (classItem) =>
      classItem.stats.attendedSessions + classItem.stats.absentSessions >=
      classItem.stats.totalSessions
  );
