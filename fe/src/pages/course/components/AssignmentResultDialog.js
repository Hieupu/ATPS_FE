  ...
  const renderSubmissionContent = () => {
    ...
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
        ...
        <Card sx={{ p: 3 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            ...
            <InfoRow 
              ...
              value={
                <Chip 
                  label={submission.Status === 'submitted' ? 'Đúng hạn' : 'Nộp muộn'}
          ...
          <Card sx={{ p: 3 }}>
            ...
            {submission.DurationSec && (
              <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                Thời lượng: {Math.floor(submission.DurationSec / 60)}:{(submission.DurationSec % 60).toString().padStart(2, '0')}
              </Typography>
            )}
          </Card>
        )}
        ...
        {!results.canShowAnswers && assignmentData.ShowAnswersAfter === 'after_deadline' && (
          <Alert severity="info">
            Đáp án sẽ được hiển thị sau hạn nộp: {formatDate(assignmentData.Deadline)}
          </Alert>
        )}

        {!results.canShowAnswers && assignmentData.ShowAnswersAfter === 'never' && (
          <Alert severity="info">
...
const formatAnswer = (question, answer) => {
  ...
  if (question.Type === 'multiple_choice') {
    const option = question.Options?.find(opt => opt.OptionID.toString() === answer);
